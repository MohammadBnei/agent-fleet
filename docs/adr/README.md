...
| [0005](0005-explicit-approval-gate.md) | Explicit human approval only gates write/edit unlock | Accepted (enforcement mechanism corrected by [0021](0021-continuous-streaming-session.md); approval *trigger* further revised by [0025](0025-continuous-session-worker-redesign.md) — word-match text fallback deleted, structured-only `/approve` is now the sole trigger. The `allowedTools`/`canUseTool` gate itself is unchanged since this ADR) |
...
| [0021](0021-continuous-streaming-session.md) | One continuous streaming-input session per task; live setPermissionMode() on approval | Accepted (prompt-split/round-cap/word-match-approval design superseded by [0025](0025-continuous-session-worker-redesign.md); the `allowedTools`/`canUseTool` live-gate mechanism this ADR verified via its own spike remains the current, unchanged foundation) |
