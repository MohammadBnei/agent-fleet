...
4. Then the source itself (`worker/src/`, `bot/src/`, `mcp-redis/src/`).

---

## 1. Locked decisions (no dedicated ADR — never really in question)

- **Bun is the sole JS runtime** across `bot/`, `worker/`, `mcp-redis/` —
  no build step, TypeScript sources run directly. No npm/pnpm/yarn
  workspace tooling; each package keeps its own `package.json`/`bun.lock`.
...
- **One repo, one version/CHANGELOG.** `release.yml` runs from the repo
  root, not per-package — the fleet ships as one unit even though `bot/`,
  `worker/`, `mcp-redis/` are deployed as separate images.
...
- **Pub/sub for the planning transcript.** Durable RPUSH + polled LRANGE
  only — see `adr/0001`.
