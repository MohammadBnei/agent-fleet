...
// ListWorkerPodsByLabel is the reconcile loop's terminal-phase GC pass
// (docs/adr/0019/0020 — the provisioner holds no DB credentials, so
// "which worker pods exist and what state are they in" comes straight
// from Kubernetes, not a Postgres-tracked list).
func (c *Client) ListWorkerPodsByLabel(ctx context.Context) ([]LiveWorkerPod, error) {
	list, err := c.Core.CoreV1().Pods(c.Namespace).List(ctx, metav1.ListOptions{
	...
	out := make([]LiveWorkerPod, 0, len(list.Items))
	for _, pod := range list.Items {
		taskID := pod.Labels[TaskIDLabel]
		if taskID == "" {
			continue
		}
		out = append(out, LiveWorkerPod{TaskID: taskID, PodName: pod.Name, Phase: string(pod.Status.Phase)})
	}
	return out, nil
}
