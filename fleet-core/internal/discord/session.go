...
// relay) into fleet-core — no separate Deployment, since it shares
// fleet-core's trust boundary (no cluster RBAC) with transcript
...
type Client struct {
	...
	e2e       *e2eclient.Client
	guildID   string
	channelID string
	appID     string
}

func New(cfg config.Config, taskStore *tasks.Store, transcr transcript.Store, e2e *e2eclient.Client) (*Client, error) {
