...
test("liveness outranks pod phase — never SCHEDULED next to DONE", () => {
  const b = sessionBadge(session({ podPhase: "POD_PHASE_SCHEDULED", liveState: "done" }));
...
test("a stopped session with no live pod carries no badge at all", () => {
  expect(sessionBadge(session({ podPhase: "POD_PHASE_TERMINATED", liveState: "" }))).toBeNull();
});
...
test("archived and swept are the states that outlive the pod", () => {
  expect(sessionBadge(session({ podPhase: "POD_PHASE_TERMINATED", archivedAt: "2026-08-01T00:00:00Z" }))?.label).toBe("ARCHIVED");
  expect(sessionBadge(session({ podPhase: "POD_PHASE_TERMINATED", sweptAt: "2026-08-01T00:00:00Z" }))?.label).toBe("SWEPT");
});
...
test("a torn-down session that recorded an error says so", () => {
  const b = sessionBadge(session({ podPhase: "POD_PHASE_TERMINATED", lastError: "pod never produced output" }));
...
test("needing a human outranks everything, including a crash", () => {
  const b = sessionBadge(session({ podPhase: "POD_PHASE_CRASHED", liveState: "blocked", pendingDecisions: 1 }));
  expect(b?.label).toBe("NEEDS YOU");
});

test("a crash outranks ordinary progress", () => {
  expect(sessionBadge(session({ podPhase: "POD_PHASE_CRASHED", liveState: "working" }))?.label).toBe("CRASHED");
});

test("provisioning keeps its sub-step, which is the difference between progress and a wedge", () => {
  const b = sessionBadge(session({ podPhase: "POD_PHASE_PROVISIONING", podMessage: "cloning repo", liveState: "unknown" }));
...
test("a live pod's state outranks archived/swept, which describe a session at rest", () => {
  expect(sessionBadge(session({ liveState: "working", archivedAt: "2026-08-01T00:00:00Z" }))?.label).toBe("WORKING");
  expect(sessionBadge(session({ liveState: "blocked", sweptAt: "2026-08-01T00:00:00Z" }))?.label).toBe("NEEDS YOU");
});
