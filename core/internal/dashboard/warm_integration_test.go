...
func TestServer_Warm_BootsNewPod(t *testing.T) {
	...
	s := NewServer(taskStore, &recordingStore{}, nil, repoStore, nil, provisioner, nil, 5, nil)
...
func TestServer_Warm_AtCapacity_ResourceExhausted(t *testing.T) {
	...
	fake, provisioner := newFakeProvisioner(t)
	s := NewServer(taskStore, &recordingStore{}, nil, repoStore, nil, provisioner, nil, cap, nil)
...
func TestServer_Discuss_AutoWarmsIdleSession(t *testing.T) {
	...
	s := NewServer(taskStore, store, nil, repoStore, nil, provisioner, nil, 5, nil)
