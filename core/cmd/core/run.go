...
func run(ctx context.Context, cfg config.Config, pool *pgxpool.Pool) error {
	...
	agentfleetv1.RegisterCoreServiceServer(grpcServer, coreserver.New(activityStore, taskStore, journalStore, provisioner, files))
	...
	dashboardSvc := dashboard.NewServer(taskStore, activityStore, journalStore, repoStore, snippetStore, provisioner, files, hub, cfg.MaxInFlight)
