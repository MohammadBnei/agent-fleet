...
| Worker agent runtime | `@anthropic-ai/claude-agent-sdk` `query()` in **streaming-input mode** (not `claude -p` headless, not the plain-string form) — one continuous session spans planning and implementation, model `claude-opus-4-8` (see `docs/adr/0021`) |
| Runtime (`core/`, `provisioner/`, `sidecar/`) | Go, `go.work` workspace, `golangci-lint` |
| Discord | `discordgo` (Go, in `core/`) |
| Coordination | Postgres `planning_transcript` — pull/cursor reads via `core`'s gRPC `CoreService`, real idempotency-keyed dedup, not pub/sub (see `docs/adr/0013`/`0020`) |
...
| `core/` | Go — Discord ingress + dispatch loop (claims tasks, commands the provisioner) + `CoreService` gRPC server + planning-transcript coordination + Loki queries + the web dashboard's ConnectRPC API and static SPA, one binary, **no cluster RBAC**, sole Postgres-credential holder |
...
| `worker/` | The Claude Code worker (TS/Bun) — **single-shot**: one task per pod, one continuous streaming-input session spanning planning+implementation, then exits. Talks only to its own pod's `localhost` sidecar |
...
| `db/schema.sql` | Shared `tasks`/`knowledge_journal`/`planning_transcript` tables (`agentfleetdb`). `e2e_sessions` still exists in the schema but is dead code — see `docs/ARCHITECTURE.md` §6 |
...
- **One task per pod, single-shot, spawned on demand by the provisioner —
  not one persistent pod per target repo.** Fleet-wide concurrency (default
  cap 5), not one-task-per-repo. Git worktree isolation happens on the one
  shared PVC, keyed by task ID, before the pod even exists (`docs/adr/
  0019`).
...
- Write/edit tools unlock only after Mohammad's **explicit** approval —
  never inferred from silence or round completion. Enforced by two
  independent layers: `Write`/`Edit` absent from `allowedTools` for the
  whole session, plus a live `canUseTool` callback gating on an in-memory
  `approved` flag, flipped by a live `query.setPermissionMode()` call on
  approval — no session teardown at the boundary (`docs/adr/0021`).
