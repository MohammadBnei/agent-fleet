...
| `dashboard/` | React + Vite + TypeScript + Tailwind/DaisyUI SPA, talks to `core` via a generated ConnectRPC client, built into `core`'s binary — not deployed on its own. Rewritten as a console in `docs/adr/0042`: five full-width views (no sidebar), decisions answerable **from the list**, a five-tier feed with one three-way DENSITY control, desktop decision spine / mobile docked decision, two themes, and an installable PWA whose service worker deliberately caches **no** fleet state (a cached task list would have someone answering a resolved decision). Desktop and mobile share `SessionFeed`/`SessionPanels`/`DecisionInline` — put anything both surfaces need there, not in one of them |
